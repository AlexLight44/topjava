https://app.codacy.com/organizations/gh/AlexLight44//topjava/settings
...
- **ВНИМАНИЕ: выбирайте для проекта простой путь без пробелов и русских букв, например, `c:\projects\topjava\` (
...
> Вместо Fork можно
> сделать [клонирование проекта](https://github.com/JavaOPs/topjava//Git#user-content-Клонирование-проекта): он не будет
> привязан к исходному https://github.com/JavaOPs/topjava и у него не будет истории.
...
- через Stream API за 1 проход по исходному списку `meals.stream()`
    ...
    - возможно дополнительные проходы по частям списка, при этом превышение должно считаться один раз для всего
      подсписка. Те например нельзя разбить список на 2 подсписка с четными и нечетными датами и затем их объединить, с
      подсчетом превышения для каждого элемента.
